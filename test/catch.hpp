...
#define INTERNAL_CATCH_METHOD_AS_TEST_CASE(QualifiedMethod, ...)                                   \
  namespace {                                                                                      \
    Catch::AutoReg INTERNAL_CATCH_UNIQUE_NAME(autoRegistrar)(&QualifiedMethod,                     \
                                                             "&" #QualifiedMethod,                 \
                                                             Catch::NameAndDesc(__VA_ARGS__),      \
                                                             CATCH_INTERNAL_LINEINFO);             \
  }
...
namespace Detail {
  ...
  std::string rangeToString(InputIterator first, InputIterator last) {
    ...
    if (first != last) {
      ...
      for (++first; first != last; ++first) oss << ", " << Catch::toString(*first);
    }
    oss << " }";
    return oss.str();
  }
...
#define INTERNAL_CATCH_TEST(expr, resultDisposition, macroName)                                    \
  do {                                                                                             \
    ...
    } catch (...) { __catchResult.useActiveException(Catch::ResultDisposition::Normal); }          \
    INTERNAL_CATCH_REACT(__catchResult)                                                            \
  } while (Catch::isTrue(false && (expr))) // expr here is never evaluated at runtime but it forces
...
#define INTERNAL_CATCH_NO_THROW(expr, resultDisposition, macroName)                                \
  do {                                                                                             \
    ...
    } catch (...) { __catchResult.useActiveException(resultDisposition); }                         \
    INTERNAL_CATCH_REACT(__catchResult)                                                            \
...
#define INTERNAL_CATCH_THROWS(expr, resultDisposition, macroName)                                  \
  do {                                                                                             \
    ...
    if (__catchResult.allowThrows()) try {                                                         \
      ...
      } catch (...) { __catchResult.captureResult(Catch::ResultWas::Ok); }                         \
    else                                                                                           \
      __catchResult.captureResult(Catch::ResultWas::Ok);                                           \
    INTERNAL_CATCH_REACT(__catchResult)                                                            \
  } while (Catch::alwaysFalse())
...
#define INTERNAL_CATCH_THROWS_AS(expr, exceptionType, resultDisposition, macroName)                \
  do {                                                                                             \
    ...
    if (__catchResult.allowThrows()) try {                                                         \
        expr;                                                                                      \
        __catchResult.captureResult(Catch::ResultWas::DidntThrowException);                        \
      } catch (exceptionType) { __catchResult.captureResult(Catch::ResultWas::Ok); } catch (...) { \
  ...
  do {                                                                                             \
    ...
    try {                                                                                          \
      ...
      __catchResult.setLhs(Catch::toString(arg))                                                   \
          .setRhs(matcherAsString == Catch::Detail::unprintableString ? #matcher                   \
                                                                      : matcherAsString)           \
          .setOp("matches")                                                                        \
...
public:
  ...
  operator T() const {
    ...
    return T(); // Suppress spurious "not all control paths return a value" warning in Visual Studio
...
namespace Detail {
  ...
  public:
    ...
    Approx(Approx const& other)
        : m_epsilon(other.m_epsilon), m_scale(other.m_scale), m_value(other.m_value) {}
  ...
  namespace Impl {
    ...
    namespace StdString {
      ...
      struct Contains : MatcherImpl<Contains, std::string> {
        ...
        virtual std::string toString() const { return "contains: \"" + m_substr + "\""; }
      ...
      struct StartsWith : MatcherImpl<StartsWith, std::string> {
        ...
        virtual bool match(std::string const& expr) const { return expr.find(m_substr) == 0; }
        virtual std::string toString() const { return "starts with: \"" + m_substr + "\""; }
      ...
      struct EndsWith : MatcherImpl<EndsWith, std::string> {
        ...
        virtual std::string toString() const { return "ends with: \"" + m_substr + "\""; }
...
public:
  ...
  Option(Option const& _other) : nullableValue(_other ? new (storage) T(*_other) : NULL) {}
...
public:
  ...
  Config(ConfigData const& data) : m_data(data), m_os(Catch::cout().rdbuf()) {
    if (!data.testsOrTags.empty()) {
      ...
      for (std::size_t i = 0; i < data.testsOrTags.size(); ++i) parser.parse(data.testsOrTags[i]);
  ...
  void setFilename(std::string const& filename) { m_data.outputFilename = filename; }
  ...
  void setStreamBuf(std::streambuf* buf) { m_os.rdbuf(buf ? buf : Catch::cout().rdbuf()); }
...
const unsigned int consoleWidth = 80;
  ...
  struct TextAttributes {
    TextAttributes()
        : initialIndent(std::string::npos), indent(0), width(consoleWidth - 1), tabChar('\t') {}
    ...
    Text(std::string const& _str, TextAttributes const& _attr = TextAttributes()) : attr(_attr) {
      ...
      while (!remainder.empty()) {
        ...
        if (width == remainder.size()) {
          ...
          spliceLine(indent, remainder, width);
          if (width <= 1 || remainder.size() != 1) remainder = remainder.substr(1);
          indent = _attr.indent;
        } else {
...
struct Parser {
  ...
  void parseIntoTokens(int argc,
    ...
    for (int i = 1; i < argc && argv[i] != doubleDash; ++i) parseIntoTokens(argv[i], tokens);
  }
...
public:
  ...
  CommandLine(CommandLine const& other)
    ...
    if (other.m_floatingArg.get()) m_floatingArg.reset(new Arg(*other.m_floatingArg));
  }
  ...
  ArgBuilder operator[](int position) {
    ...
    if (position > m_highestSpecifiedArgPosition) m_highestSpecifiedArgPosition = position;
  ...
  ArgBuilder operator[](UnpositionalTag) {
    if (m_floatingArg.get()) throw std::logic_error("Only one unpositional argument can be added");
  ...
  void optUsage(std::ostream& os,
    ...
    typename std::vector<Arg>::const_iterator itBegin = m_options.begin(), itEnd = m_options.end(),
                                              it;
    std::size_t maxWidth = 0;
    for (it = itBegin; it != itEnd; ++it) maxWidth = (std::max)(maxWidth, it->commands().size());
  ...
  std::vector<Parser::Token> parseInto(int argc, char const* const* argv, ConfigT& config) const {
    ...
    if (lastSlash != std::string::npos) processName = processName.substr(lastSlash + 1);
      ...
      for (; it != itEnd; ++it) {
        ...
        try {
          if ((token.type == Parser::Token::ShortOpt && arg.hasShortName(token.data)) ||
            ...
            } else { arg.boundField.setFlag(config); }
            break;
          }
  ...
  void validate() const {
    ...
    for (typename std::vector<Arg>::const_iterator it = m_options.begin(), itEnd = m_options.end();
         it != itEnd;
         ++it)
      it->validate();
  }
...
inline void abortAfterX(ConfigData& config, int x) {
  if (x < 1) throw std::runtime_error("Value after -x or --abortAfter must be greater than zero");
  config.abortAfter = x;
}
...
inline void setRngSeed(ConfigData& config, std::string const& seed) {
  if (seed == "time") { config.rngSeed = static_cast<unsigned int>(std::time(0)); } else {
    std::stringstream ss;
    ss << seed;
    ss >> config.rngSeed;
    if (ss.fail())
...
inline void loadTestNamesFromFile(ConfigData& config, std::string const& _filename) {
  ...
  if (!f.is_open()) throw std::domain_error("Unable to load input file: " + _filename);

  std::string line;
  while (std::getline(f, line)) {
    line = trim(line);
    if (!line.empty() && !startsWith(line, "#")) addTestOrTags(config, "\"" + line + "\",");
  }
}
...
inline Clara::CommandLine<ConfigData> makeCommandLineParser() {
  ...
  cli["-f"]["--input-file"].describe("load test names to run from a file").bind(
      &loadTestNamesFromFile, "filename");

  // Less common commands which don't have a short form
  cli["--list-test-names-only"].describe("list all/matching test cases names only").bind(
      &ConfigData::listTestNamesOnly);
  ...
  cli["--rng-seed"].describe("set a specific seed for random numbers").bind(&setRngSeed,
                                                                            "'time'|number");
...
inline std::size_t listTags(Config const& config) {
  ...
  for (std::map<std::string, TagInfo>::const_iterator countIt = tagCounts.begin(),
    ...
    Text wrapper(countIt->second.all(),
                 TextAttributes().setInitialIndent(0).setIndent(oss.str().size()).setWidth(
                     CATCH_CONFIG_CONSOLE_WIDTH - 10));
...
inline std::size_t listReporters(Config const& /*config*/) {
  ...
  for (it = itBegin; it != itEnd; ++it) maxNameLen = (std::max)(maxNameLen, it->first.size());

  for (it = itBegin; it != itEnd; ++it) {
    Text wrapper(it->second->getDescription(),
                 TextAttributes().setInitialIndent(0).setIndent(7 + maxNameLen).setWidth(
                     CATCH_CONFIG_CONSOLE_WIDTH - maxNameLen - 8));
    Catch::cout() << "  " << it->first << ":" << std::string(maxNameLen - it->first.size() + 2, ' ')
                  << wrapper << "\n";
  }
  Catch::cout() << std::endl;
...
inline Option<std::size_t> list(Config const& config) {
  ...
  if (config.listReporters()) listedCount = listedCount.valueOr(0) + listReporters(config);
  return listedCount;
}
...
namespace SectionTracking {
  ...
  public:
    ...
    bool currentSectionHasChildren() const { return m_currentSection->hasChildren(); }
    bool isCompleted() const { return m_testCase.runState() == TrackedSection::Completed; }

    class Guard {
    public:
      Guard(TestCaseTracker& tracker) : m_tracker(tracker) { m_tracker.enterTestCase(); }
...
private: // IResultCapture
  virtual void assertionEnded(AssertionResult const& result) {
    ...
    } else if (!result.isOk()) { m_totals.assertions.failed++; }
...
private:
  void runCurrentTest(std::string& redirectedCout, std::string& redirectedCerr) {
    ...
    try {
      ...
      } else { invokeActiveTestCase(); }
      duration = timer.getElapsedSeconds();
    } catch (TestFailureException&) {
      // This just means the test was aborted due to failure
    } catch (...) { makeUnexpectedResultBuilder().useActiveException(); }
    handleUnfinishedSections();
    m_messages.clear();
...
public:
  ...
  Totals runTests() {
    ...
    if (!testSpec.hasFilters())
      testSpec =
          TestSpecParser(ITagAliasRegistry::get()).parse("~[.]").testSpec(); // All not hidden tests
...
private:
  ...
  void makeReporter() {
    std::string reporterName =
        m_config->getReporterName().empty() ? "console" : m_config->getReporterName();
...
public:
  ...
  virtual std::vector<TestCase> const& getAllTests() const { return m_functionsInOrder; }

  virtual std::vector<TestCase> const& getAllNonHiddenTests() const { return m_nonHiddenFunctions; }
...
public:
  ...
  virtual std::string translateActiveException() const {
    try {
      ...
      return msg;
    } catch (...) { return tryTranslators(m_translators.begin()); }
  }
  ...
  std::string tryTranslators(std::vector<const IExceptionTranslator*>::const_iterator it) const {
    ...
    try {
      return (*it)->translate();
    } catch (...) { return tryTranslators(it + 1); }
  }
...
namespace {
  ...
  public: // IRegistryHub
    RegistryHub() {}
    virtual IReporterRegistry const& getReporterRegistry() const { return m_reporterRegistry; }
    virtual ITestCaseRegistry const& getTestCaseRegistry() const { return m_testCaseRegistry; }
...
const char* NotImplementedException::what() const CATCH_NOEXCEPT { return m_what.c_str(); }
...
namespace Detail {
  ...
  std::string rawMemoryToString(const void* object, std::size_t size) {
    ...
    for (; i != end; i += inc) os << std::setw(2) << static_cast<unsigned>(bytes[i]);
    return os.str();
  }
}
...
std::string toString(char* const value) { return Catch::toString(static_cast<const char*>(value)); }
...
std::string toString(signed char value) { return toString(static_cast<char>(value)); }

std::string toString(unsigned char value) { return toString(static_cast<char>(value)); }
...
std::string toString(NSObject* const& nsObject) { return toString([nsObject description]); }
#endif
...
void ResultBuilder::captureExpression() {
  ...
  if (!result.isOk()) {
    if (getCurrentContext().getConfig()->shouldDebugBreak()) m_shouldDebugBreak = true;
...
struct StreamingReporterBase : SharedImpl<IStreamingReporter> {
  ...
  virtual void testGroupStarting(GroupInfo const& _groupInfo) { currentGroupInfo = _groupInfo; }

  virtual void testCaseStarting(TestCaseInfo const& _testInfo) { currentTestCaseInfo = _testInfo; }
  virtual void sectionStarting(SectionInfo const& _sectionInfo) {
    m_sectionStack.push_back(_sectionInfo);
  }

  virtual void sectionEnded(SectionStats const& /* _sectionStats */) { m_sectionStack.pop_back(); }
...
struct CumulativeReporterBase : SharedImpl<IStreamingReporter> {
  ...
  virtual void sectionStarting(SectionInfo const& sectionInfo) {
    ...
    if (m_sectionStack.empty()) {
      ...
      SectionNode::ChildSections::const_iterator it = std::find_if(parentNode.childSections.begin(),
...
public:
  ...
  public:
    ...
    ScopedElement(ScopedElement const& other) : m_writer(other.m_writer) { other.m_writer = NULL; }
  ...
  XmlWriter& endElement() {
    ...
    if (m_tagIsOpen) {
      stream() << "/>\n";
      m_tagIsOpen = false;
    } else { stream() << m_indent << "</" << m_tags.back() << ">\n"; }
    m_tags.pop_back();
    return *this;
  }
...
public:
  XmlReporter(ReporterConfig const& _config) : StreamingReporterBase(_config), m_sectionDepth(0) {}
...
public: // StreamingReporterBase
  ...
  virtual void sectionStarting(SectionInfo const& sectionInfo) {
    ...
    if (m_sectionDepth++ > 0) {
      m_xml.startElement("Section").writeAttribute("name", trim(sectionInfo.name)).writeAttribute(
  ...
  virtual bool assertionEnded(AssertionStats const& assertionStats) {
    ...
    if (!m_config->includeSuccessfulResults() && isOk(assertionResult.getResultType())) return true;
...
private:
  ...
  void printTotals(Totals const& totals) {
    if (totals.testCases.total() == 0) {
      ...
      columns.push_back(SummaryColumn("", Colour::None).addRow(totals.testCases.total()).addRow(
          totals.assertions.total()));
