...
namespace
{
  ...
  using x = decltype(fused::slice<1>(fused::tuple<_0, _1>{}));
  static_assert(std::is_same<x, fused::tuple<_0>>::value);
  using y = decltype(fused::slice<2>(fused::tuple<_0, _1>{}));
  ...
  TEST_CASE("canTakeElementsFromTuple", "[fused::take]")
  {
    ...
    auto b = fused::slice<1>(a);
    auto c = fused::slice<2>(a);
    auto d = fused::slice<3>(a);
